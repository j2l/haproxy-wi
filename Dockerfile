...
RUN       apt-get update -qq && apt-get upgrade -y --with-new-pkgs -qq && \
	  apt-get install -y --no-install-recommends --qq apt-utils apt-transport-https ca-certificates
	  
RUN	  apt-get install -y --no-install-recommends --qq gnupg curl wget bash nano net-tools dnsutils git \
